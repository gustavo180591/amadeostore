<script lang="ts">
	...
	let totalItems = $derived(() => cartItems.reduce((total, item) => total + item.quantity, 0));
	...
	function getStockStatus(item: CartItem): { text: string; class: string } {
		...
		if (item.stock <= 3)
			return { text: `¡Últimas ${item.stock} unidades!`, class: 'text-orange-600' };
											...
											<div class="flex items-start justify-between">
												<div>
													<h3 class="text-base font-medium text-gray-900">
														<a href={resolve(`/products/${item.slug}`)} class="hover:text-blue-600">
															{item.name}
														</a>
													</h3>
													{#if item.variant}
												...
												<div class="flex items-center space-x-2">
													...
													<div class="flex items-center rounded-md border border-gray-300">
														<button
															...
															class="rounded-l-md px-3 py-1 text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
														>
															-
														</button>
														<input
														...
														<button
															...
															class="rounded-r-md px-3 py-1 text-gray-600 hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
														>
															+
														</button>
													</div>
						...
						<div class="space-y-4 px-6 py-4">
							...
							<div class="space-y-3 pt-4">
								...
								<button
									...
									class="flex w-full items-center justify-center gap-2 rounded-md bg-green-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50"
								>
									{#if isCheckingOut}
							...
							<div class="mt-4 rounded-md bg-blue-50 p-3">
								<p class="text-sm text-blue-800">
									...
									Al hacer clic en "Finalizar Pedido", se abrirá WhatsApp con tu pedido pre-cargado para
									que lo envíes y coordines la entrega.
								</p>
							</div>
						</div>
					</div>
