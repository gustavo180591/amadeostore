				...
				<div class="flex items-center">
					...
					<div class="ml-5 w-0 flex-1">
						<dl>
							...
							<dd class="text-lg font-medium text-gray-900">{formatCurrency(stats?.averageOrderValue || 0)}</dd>
						</dl>
					</div>
				</div>
			</div>
	...
	<div class="bg-white shadow rounded-lg mb-6">
		<div class="px-4 py-5 sm:p-6">
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
				...
				<div>
					<label for="paymentStatus" class="block text-sm font-medium text-gray-700">Estado Pago</label>
					<select
						id="paymentStatus"
				...
				<tbody class="bg-white divide-y divide-gray-200">
					{#each filteredOrders as order (order.id)}
						<tr>
							...
							<td class="px-6 py-4 whitespace-nowrap">
								<span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full {getPaymentStatusClass(order.paymentStatus)}">
									{order.paymentStatus}
								</span>
							</td>
					...
					<nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
						...
						{#each Array.from({ length: pagination.totalPages }, (_, i) => i + 1) as page}
							<a
								...
								class="relative inline-flex items-center px-4 py-2 border text-sm font-medium {page === pagination.currentPage ? 'z-10 bg-green-50 border-green-500 text-green-600' : 'bg-white border-gray-300 text-gray-500 hover:bg-gray-50'}"
							>
								{page}
							</a>
						{/each}
