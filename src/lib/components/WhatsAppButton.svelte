...
<button
	...
	class="inline-flex transform items-center justify-center rounded-lg font-medium transition-all duration-200 hover:scale-105 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:outline-none {sizeClasses[
		size
...
{#if variant === 'floating'}
	<div class="fixed right-6 bottom-6 z-40">
		<button
			onclick={handleWhatsAppClick}
			class="flex h-14 w-14 items-center justify-center rounded-full bg-green-600 text-white shadow-lg transition-all duration-200 hover:scale-110 hover:bg-green-700 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 focus:outline-none"
			disabled={itemCount === 0}
			title={itemCount === 0 ? 'Agrega productos al carrito para pedir' : 'Pedir por WhatsApp'}
		>
			{#if itemCount > 0}
